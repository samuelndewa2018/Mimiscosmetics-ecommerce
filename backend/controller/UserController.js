...
exports.forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    await sendMail({
      email: user.email,
      subject: `Ecommerce E-commerce Password Recovery`,
      message,
    });
...
/// receive email in contact us form

exports.receiveEmail = catchAsyncErrors(async (req, res, next) => {
  ...
  mailgun()
    ...
    .send(
      {
        from: `${email}`,
        to: "Samuel Githinji <samuelndewa2018@gmail.com>",
        subject: `${subject}`,
        html: `<p> ${name} says:, ${message}</p>`,
      },
      (error, body) => {
        if (error) {
          console.log(error);
          res
            .status(500)
            .send({ message: "Error in sending email. Are you offline?" });
        } else {
          console.log(body);
          res.send({
            message:
              "Email sent successfully. Thanks for your report we will reply to it very soon...",
          });
        }
      }
    );
